...
class FrameDropMonitor {
  ...
  static const Duration _frameDropThreshold = Duration(milliseconds: 32); // 2 frames
  ...
  static void markHeavyOperation(String operation) {
    if (_isMonitoring) {
      _heavyOperations.add('${DateTime.now().millisecondsSinceEpoch}: $operation');
      if (kDebugMode) {
  ...
  static void _printFinalReport() {
    ...
    final averageFrameTime = _calculateAverageFrameTime();
    final fps = (1000 / averageFrameTime.inMilliseconds).round();
    final frameDropPercentage = (_skippedFrames / _frameTimes.length * 100).toStringAsFixed(2);
    ...
    if (_skippedFrames > _frameTimes.length * 0.1) {
      ...
      debugPrint('  • Profile with Flutter Inspector for widget rebuild analysis');
    }
